		...
		if strings.HasSuffix(file, ".jpg") == true || strings.HasSuffix(file, ".jpeg") == true || strings.HasSuffix(file, ".png") == true {
			...
			if len(tempFaces) >= 1 {
				if pic == tempFaces[0].Descriptor {
					...
					//Writing shell script open "file"
					WriteToFile("/Users/oliviakumar/Documents/Fall19/Senior Team/vmware_edgewalk/model-goface/open-pic.sh", file)

					fmt.Println("Opening image used to train...")
					//Executing shell script
					c := exec.Command("/Users/oliviakumar/Documents/Fall19/Senior Team/vmware_edgewalk/model-goface/open-pic.sh")
...
func test(rec *face.Recognizer, picName string) {
